...
Node<K, V>::Node(const K k, const V v, int level) {
    ...
    this->forward = new Node<K, V>*[level + 1];  //数组大小/树高leval+1
...
void SkipList<K, V>::delete_element(K key) {
    ...
    if (current != nullptr && current->get_key() == key) {
        ...
        for (int i = current->node_level; i >= 0; i--) {  //替换简洁代码
            update[i]->forward[i] = current->forward[i];
        }
        ...
        delete current;  //删除该节点信息
        _element_count--;
    }
    else {
        std::cout << "The key is not exist: " << key << std::endl;
    }
...
int SkipList<K, V>::get_random_level() {
    int k = 0;  //应该从0层开始，否则level0,1完全相同了
    while (rand() % 2) {
        k++;
    }
